"""
Focused on IOPS-bound regime (BS <= 32 KB).

Aggressively removes block sizes > 32 KB before training.  Uses
unified (non-regime) model training with random stratified splitting.
Runs two scenarios: 60/40 and 70/30 splits.  Excludes SATA/SDA devices.

Usage
-----
    python 10_iops_bound_focused.py <data_path>
"""
...
SEED = 42
PLOT_EXT = "pdf"
BS_UPPER_LIMIT = 32  # Only keep block sizes <= this value
...
def train_models(df_train, features):
    """Train one model set per device."""
...
def main():
    parser = argparse.ArgumentParser(
        description="IOPS-bound focused training (BS <= 32 KB).",
    )
    ...
    print(f"Filtering out block sizes > {BS_UPPER_LIMIT}KB...")
    df_eng = df_eng[df_eng["block_size_kb"] <= BS_UPPER_LIMIT]

    if df_eng.empty:
        print(f"CRITICAL: No data left after filtering BS <= {BS_UPPER_LIMIT}KB. Exiting.")
        return
    ...
    for train_frac in split_ratios:
        ...
        print(f" RUNNING EXPERIMENT: {int(train_frac * 100)}% Train / "
              f"{int(test_frac * 100)}% Test (BS <= {BS_UPPER_LIMIT}KB)")
        print("=" * 60)

        plot_dir = f"barcharts_random_leq32k_{ratio_name}"
        ...
        if res is not None:
            ...
            create_summary_plot(
                agg, plot_dir,
                title_suffix=(f"BS <= {BS_UPPER_LIMIT}k | "
