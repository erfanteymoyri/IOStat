"""
Deterministic training with mentor's visualization style.

Same training logic as 05_deterministic_training (trains on
AllDevices.xlsx, tests on limit100.xlsx) but uses the mentor's 2-panel
grid layout with larger font sizes.

Usage
-----
    python 11_deterministic_mentor_viz.py <train_data_path> <test_data_path>
"""
...
# ------------------------------------------------------------------ #
# Configuration (mentor font sizes)
# ------------------------------------------------------------------ #
...
def engineer_features(df):
    """Extract device name from file path column."""
    df = df.copy()
...
# ------------------------------------------------------------------ #
# Visualization – mentor's 2-panel grid plots
# ------------------------------------------------------------------ #
...
def main():
    parser = argparse.ArgumentParser(
        description="Deterministic training with mentor's visualization style.",
    )
    parser.add_argument("train_path",
                        help="Path to the training data file (e.g. AllDevices.xlsx)")
    parser.add_argument("test_path",
                        help="Path to the test data file (e.g. limit100.xlsx)")
    args = parser.parse_args()

    train = engineer_features(read_and_prep_data(args.train_path, False))
    test = engineer_features(read_and_prep_data(args.test_path, True))
