"""
Hardware-Aware Smart Hybrid Model.

Introduces SmartHybridRegressor, a meta-estimator that routes
predictions based on block size: SVR for small (<=8 KB), RF for
mid (8-16 KB) and large (>16 KB).  Adds SMART_HYBRID to the
standard model order with gold colour and star hatch.

Train on powers of 2, test on non-standard block sizes.
Excludes SATA devices.

Usage
-----
    python 07_smart_hybrid_model.py <data_path>
"""
...
from config import (
    HYPERPARAMS, MODEL_ORDER_WITH_HYBRID,
    HATCH_MAP, BAR_COLORS, LEGEND_LABELS,
...
# ------------------------------------------------------------------ #
# Smart Hybrid Meta-Model
# ------------------------------------------------------------------ #
class SmartHybridRegressor(BaseEstimator, RegressorMixin):
    """Route predictions through different models based on block size."""
...
def load_unified_data(path):
    """Load and aggregate data (MAX strategy per config)."""
...
def engineer_features(df):
    """Extract device name and exclude sata."""
    df = df.copy()
...
def train_models(df, features):
    """Train all five model families per device."""
...
def evaluate_and_predict(df, models, features):
    """Predict on the test set using per-device trained models."""
...
# ------------------------------------------------------------------ #
MODEL_ORDER = MODEL_ORDER_WITH_HYBRID


def create_plots(df, error_line_color="navy", error_line_width=2.5,
                 error_line_alpha=0.9):
    """Two-panel plot with 6 models (including SMART_HYBRID)."""
    ...
    for (dev, bs), g in df.groupby(["device", "block_size_kb"]):
        ...
        fig.legend(handles=legend_handles, loc="upper center",
                   bbox_to_anchor=(0.53, 1.08), ncol=3,
...
def create_summary_plot(results_df, bar_width=0.16):
    """Grouped bar chart of MAPE per device, including SMART_HYBRID."""
    ...
    ax.legend(handles=legend_handles, loc="upper center",
              bbox_to_anchor=(0.5, 1.25), ncol=5,
...
def main():
    parser = argparse.ArgumentParser(
        description="Hardware-Aware Smart Hybrid Model for IOPS prediction.",
    )
